...
def test_dynamic_power_profile_wording_reports_hardware_scope(machine):
    ...
    assert adapter.warnings == [
        (
            ...
            "hardware evidence from three one-layer vector coupons on a Boss "
            ...
            "its reduced span. A corrected 15%-5%-15% coupon with explicit "
            "restoration produced the operator-reported result 'a ~30mm "
            "line, a gap, and a ~30mm line.' This establishes only that exact "
            "restore subset, not calibrated power or zero output. The broad "
            "profile remains research-only and every other dynamic-power "
    ...
    assert profile_var.get_display_for_value("dynamic-power-research") == (
        ...
        "one restore subset operator-observed"
    )
    assert profile_var.description == (
        "Advanced profiles are evidence-limited; planned-path has narrow "
        "positive hardware observations, dynamic power has "
        "one exact operator-observed restore subset, but its broad profile "
        "remains research-only and other combinations are unvalidated; the "
        "remaining profiles are offline-only"
    )
